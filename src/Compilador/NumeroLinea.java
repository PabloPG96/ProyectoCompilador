...
//
    ...
    // Añade un método para obtener el texto con los números de línea
    public String getTextoConNumerosDeLinea() {
        StringBuilder textoConNumerosDeLinea = new StringBuilder();
        Element root = component.getDocument().getDefaultRootElement();
        int lines = root.getElementCount();

        for (int i = 0; i < lines; i++) {
            textoConNumerosDeLinea.append(i + 1).append(": ").append(getLineText(i)).append("\n");
        }

        return textoConNumerosDeLinea.toString();
    }

    // Añade un método para obtener el texto de una línea específica
    private String getLineText(int lineIndex) {
        ...
        try {
            return component.getDocument().getText(start, end - start);
